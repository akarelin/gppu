...
`catalog` is the only setting: an absolute folder with one subfolder per host or server, named as the Locations table names it, holding that host's `locations.json`: one row per Location as the table has it, plus `root_path`, the absolute path of the Location on that host, and `index`, where gppufs keeps that Location's index. gppufs opens the folder named after the machine it runs on. The JSON files beside the host folders are the global catalog, one per source: `sharepoint.json`, `onedrive.json`, `synology-drive.json`, `google-drive.json`, `dropbox.json`, `obsidian.json`, `git.json`. Each names its `service` and lists that service's `locations` as the lake has them, every location with its `replicas` on hosts, and for m365 its `server`, `origin`, `site` and `library`. A folder that is a replica on this host carries a `source` block with the service and what the catalog knows of the location, so `D:\Karelin` is SharePoint for m365-karelin and `D:\OneDrive - Karelin\Finance` the SharePoint library at its origin. The block shows as the Source column. gppufs derives the same index path from the Location and refuses a row that says otherwise, so the catalog cannot move an index. The examples start at the catalog: its root lists the Locations without touching them, entering one lists that Location, and Backspace walks up the Locations tree back to the catalog. A `GppuFileSystem` can still be built on one Location directly.
