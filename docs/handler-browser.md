...
`catalog` is the only setting: an absolute folder with one subfolder per host or server, named as the Locations table names it, holding that host's `locations.json`: one row per Location as the table has it, plus `root_path`, the absolute path of the Location on that host, and `index`, where gppufs keeps that Location's index. gppufs opens the folder named after the machine it runs on. The JSON files beside the host folders are the global catalog, one per service: `sharepoint.json`, `onedrive.json`, `synology-drive.json`, `google-drive.json`, `dropbox.json`, `obsidian.json`, `git.json`. Each names its `service` and lists that service's canonical `locations` as the lake has them, nested, a location carrying its children in its own `locations`, with `server`, `origin`, `site` and `library` where known. Physical copies stay separate: a host folder's `replicas.json` says where that host holds a copy of a canonical location, by service and location name, with its path and `checked_at`, the last time the folder was seen there. A folder that is a replica carries a `source` block with the service, what the catalog knows of the canonical location, and `checked_at`, so `D:\Karelin` is SharePoint for m365-karelin and `D:\OneDrive - Karelin\Finance` the SharePoint library at its origin. The block shows as the Source column. A replica of a location no service file lists is refused. gppufs derives the same index path from the Location and refuses a row that says otherwise, so the catalog cannot move an index. The examples start at the catalog: its root lists the Locations without touching them, entering one lists that Location, and Backspace walks up the Locations tree back to the catalog. A `GppuFileSystem` can still be built on one Location directly.
