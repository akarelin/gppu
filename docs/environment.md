...
`examples/config` is the reference: `_config/_ran.yaml` in the `_creekview.yaml` shape with `platforms`, `connections`, `hosts` and `locations` beside it, each carrying its templates with its rows, and `browse.py`, an `App` that lists every location with its path on this host and walks the addresses of its own section through gppufs. Run it with no arguments.
