...
CONFIG = {
  ...
  'locations': [
    {'uid': 'm365', 'name': 'M365', 'path': '', 'locations': [
...
def test_native_env_catalog_preserves_input_and_resolves_nested_templates():
  ...
  contact = catalog.location('m365-karelin-graph/alex/contacts')
  ...
  assert catalog.connection(contact['uid'])['provider'] == 'm365'
  assert catalog.path('lake', 'Contacts/person.json') == '/mnt/S1/Lake/text/Contacts/person.json'
  assert catalog.info_sync(contact['uid'])['gppu'] == contact
  assert catalog.ls_sync('lake') == []
  contact['name'] = 'Changed outside catalog'
  assert catalog.location(contact['uid'])['name'] == 'Contacts'


def test_plain_database_records_use_the_same_catalog_without_templates():
  catalog = GppuCatalog({
    'connections': {'file': {'provider': 'file'}},
    'locations': {
      'local-contacts': {'id': 123, 'path': '', 'connection': 'file', 'access': '/mnt/S1/Lake/text/Karelin'},
    },
  })
  assert catalog.location('local-contacts')['id'] == 123
  assert catalog.path('local-contacts', 'Contacts/person.json') == '/mnt/S1/Lake/text/Karelin/Contacts/person.json'
  assert catalog.ls_sync(detail=False, recurse=True) == ['local-contacts']


@pytest.mark.parametrize('relative', ['../elsewhere', 'Contacts/../../elsewhere', '/etc/passwd', 'D:/other', 'smb://s1/other'])
def test_local_path_cannot_escape_location(relative):
  with pytest.raises(ValueError):
    GppuCatalog(CONFIG).path('lake', relative)


def test_drive_root_stays_absolute():
  config = deepcopy(CONFIG)
  config['locations'][1]['access'] = 'D:\\'
  catalog = GppuCatalog(config)
  assert catalog.path('lake') == 'D:/'
  assert catalog.path('lake', 'Contacts\\person.json') == 'D:/Contacts/person.json'
...
def test_nested_paths_are_not_appended_twice():
  ...
  catalog = GppuCatalog(config)
  assert catalog.location('m365-karelin-graph/alex/contacts')['path'] == 'alex/contacts'
