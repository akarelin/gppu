...
from gppu import AsyncApp, Env, dict_from_yml, run_loop
...
class TUIApp(AsyncApp, TextualApp):
  ...
  ``with self.loading('#widget-id'): ...`` overlays Textual's loading indicator on a widget while a slow operation
  runs.
  """
  ...
  async def _process_messages(self, *args, **kwargs):
    ...
    try:
      async with self._task_scope():
        try: return await super()._process_messages(*args, **kwargs)
...
class AppScreen(Screen):
  ...
  def on_mount(self) -> None:
    ...
    self.run_worker(self._wrapped.embedded(), exit_on_error=True)
