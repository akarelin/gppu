...
| `gppu.handlers` | `identify(path)` names what a file or folder holds, `probe(path)` returns its metadata, `load(path)` returns its objects. Handlers register themselves; sessions ship registered. |
| `gppu.session` | Codex and Claude Code session logs: `Session` (ids, models, turns) and `SessionMeta` (span, models, turns). |
