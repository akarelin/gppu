...
| `gppu.handlers` | `Handler` composes a typed object loader and stats derivation into one callable. `session_handler(path)` handles a session file or folder. |
| `gppu.session` | Complete typed session objects loaded directly from files or folders, with stats derived from the loaded object. |
