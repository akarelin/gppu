...
| `gppu.handlers` | `handlers` registry, Jinja-style: `@handlers.add('name')` registers a reader that returns an object for its files and `None` otherwise. `handlers.identify(path)`, `handlers.load(path)` — a path is a file or a folder, the reader decides. |
| `gppu.session` | Session-store handler, registered on import: identifies a folder as `codex`, `claude`, `openclaw`, `hermes` or `agy`, then reads span, models and turns generically from its JSONL. |
