...
| `gppu.handlers` | `FileHandler` identifies, probes, normalizes, caches, and navigates file and folder records. Every file, folder, and archive span is derived by its handler and aggregated through the hierarchy. Files inside ZIP and RAR archives use the same records as filesystem files. `SessionHandler` reads Codex, Claude, OpenClaw, Hermes, and Agy logs and derives uid, span, turns, topic, and naming. |
