...
| [`gppu.handlers`](docs/handlers.md) | `FileHandler` identifies, probes, normalizes, caches, and navigates file and folder records from synchronous or asynchronous code. Every file, folder, archive, and Git-tracked path span is derived by its handler and aggregated through the hierarchy. Files inside ZIP, RAR, and TAR.GZ archives use the same records as filesystem files. `SessionHandler` reads native Codex, Claude Code, OpenClaw, Hermes, and Agy logs plus OpenAI and Anthropic export ZIP files. |
