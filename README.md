...
| `gppu.handlers` | `handlers` registry, Jinja-style: `@handlers.add('name')` registers a reader that returns an object for its files and `None` otherwise. `handlers.identify(path)`, `handlers.load(path)` — a path is a file or a folder, the reader decides.<br>Session-store reader, registered on import: identifies a folder as `codex`, `claude`, `openclaw`, `hermes` or `agy`, then reads its JSONL generically — `Sessions.sessions()` gives one `Session` per log with uid, span, models, turns and topic, and `Session.name` renders the naming convention. |
