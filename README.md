...
| [`gppu.handlers`](docs/handlers.md) | `FileHandler` combines explicit Markdown, CSV, log, ChatGPT, Anthropic, native-session, archive, Git, and folder handlers as mixins. Markdown retains complete YAML frontmatter and derives title, name, tags, and span; timestamped log rows derive log spans. ChatGPT and Anthropic handlers accept extracted export folders or ZIP files. Files inside ZIP, RAR, and TAR.GZ archives use the same records as filesystem files; RAR metadata comes from the platform RARLAB command. |
