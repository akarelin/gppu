...
| `gppu.handlers` | `handlers` registry, Jinja-style: `@handlers.add('name')` registers a reader that returns an object for its files and `None` otherwise. `handlers.identify(path)`, `handlers.load(path)`, `handlers.scan(folder)`. |
